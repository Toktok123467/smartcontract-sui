...
fun init(otw: JELO, ctx: &mut TxContext) {
    let (treasury, metadata) = coin::create_currency(
        ...
        b"TokTok",
        b"TokTok",
        b"Meet TokTok floating through the blockchain ocean! Dive into a world of fun, community, and rewards",
...
fun test_init() {
    ...
    scenario.next_tx(publisher);
    {
        ...
        assert!(toktok_coin.balance().value() == INITIAL_SUPPLY, EInvalidAmount);
...
fun test_lock_tokens() {
    ...
    scenario.next_tx(bob);
    {
        let coin = scenario.take_from_sender<coin::Coin<toktok>>();
